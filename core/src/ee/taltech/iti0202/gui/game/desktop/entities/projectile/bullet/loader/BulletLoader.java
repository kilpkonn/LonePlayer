...
public class BulletLoader {
    ...
    public static Bullet bulletLoader(
            SpriteBatch spriteBatch,
            World world,
            Vector2 positionRelativeToScreen,
            Vector2 destination,
            Vector2 positionRelativeToGame,
            double delta) {
        ...
        body.applyLinearImpulse(
                new Vector2(
                        (float) (destination.x - positionRelativeToScreen.x + randomValue1),
                        (float) (destination.y - positionRelativeToScreen.y + randomValue2)),
                positionRelativeToScreen,
                true);
        float angle =
                (float)
                        Math.atan2(
                                (double) body.getLinearVelocity().y,
                                (double) body.getLinearVelocity().x);
        body.setTransform(
                new Vector2(positionRelativeToGame.x, positionRelativeToGame.y),
                (angle + (float) Math.PI + ((float) Math.PI) / 1.0f));
