...
/**
 * A tweened animation is responsible for updating itself based on two given animations. The values
 * of the two given animations will get interpolated and save in this animation. When tweening two
 * animations, you have to make sure that they have the same structure. The best result is achieved
 * if bones of two different animations are named in the same way. There are still issues with
 * sprites, which are hard to resolve since Spriter does not save them in a useful order or naming
 * convention.
 *
 * @author Trixt0r
 */
