...
/**
 * This class parses a SCON file and creates a {@link Data} instance. If you want to keep track of
 * what is going on during the build process of the objects parsed from the SCON file, you could
...
public class SCONReader {
    ...
    protected void loadFolders(JSONArray folders) {
        for (int i = 0; i < folders.length(); i++) {
            ...
            Folder folder =
                    new Folder(
                            repo.getInt("id"),
                            repo.optString("name", "no_name_" + i),
                            files.length());
    ...
    protected void loadCharacterMaps(JSONArray maps, Entity entity) {
        for (int i = 0; i < maps.length(); i++) {
            ...
            CharacterMap charMap =
                    new CharacterMap(map.getInt("id"), map.optString("name", "charMap" + i));
            ...
            for (int i1 = 0; i1 < mappings.length(); i1++) {
                ...
                charMap.put(
                        ...
                        new FileReference(
                                mapping.optInt("target_folder", folder),
                                mapping.optInt("target_file", file)));
            }
        }
    }
    ...
    protected void loadAnimations(JSONArray animations, Entity entity) {
        for (int i = 0; i < animations.length(); i++) {
            ...
            Animation animation =
                    new Animation(
                            new Mainline(mainlineKeys.length()),
                            a.getInt("id"),
                            a.getString("name"),
                            a.getInt("length"),
                            a.optBoolean("looping", true),
                            timelines.length());
        ...
        for (int i = 0; i < boneRefs.length(); i++) {
            ...
            BoneRef boneRef =
                    new BoneRef(
                            ...
                            e.getInt("timeline"),
                            e.getInt("key"),
                            key.getBoneRef(e.optInt("parent", -1)));
        ...
        for (int i = 0; i < objectRefs.length(); i++) {
            ...
            ObjectRef objectRef =
                    new ObjectRef(
                            ...
                            o.getInt("timeline"),
                            o.getInt("key"),
                            key.getBoneRef(o.optInt("parent", -1)),
                            o.optInt("z_index", 0));
    ...
    protected void loadTimelineKeys(JSONArray keys, Timeline timeline) {
        for (int i = 0; i < keys.length(); i++) {
            ...
            Timeline.Key key =
                    new Timeline.Key(
                            k.getInt("id"), k.optInt("time", 0), k.optInt("spin", 1), curve);
            ...
            if (obj.optString("name", "no_name_" + i).equals("object")) {
                if (timeline.objectInfo.type == ObjectType.Sprite) {
                    ...
                    pivot =
                            new Point(
                                    obj.optFloat("pivot_x", f.pivot.x),
                                    obj.optFloat("pivot_y", f.pivot.y));
                    timeline.objectInfo.size.set(f.size);
                }
            }
            Timeline.Key.Object object;
            if (obj.optString("name", "no_name_" + i).equals("bone"))
                object =
