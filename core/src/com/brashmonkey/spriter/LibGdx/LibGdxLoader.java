...
public class LibGdxLoader extends Loader<Sprite> implements Disposable {
    ...
    protected Sprite loadResource(FileReference ref) {
        ...
        if (!f.exists())
            throw new GdxRuntimeException(
                    "Could not find file handle " + path + "! Please check your paths.");
        if (this.packer == null && this.pack)
            this.packer =
                    new PixmapPacker(
                            this.atlasWidth, this.atlasHeight, Pixmap.Format.RGBA8888, 2, true);
    ...
    protected void generatePackedSprites() {
        ...
        TextureAtlas tex =
                this.packer.generateTextureAtlas(TextureFilter.Linear, TextureFilter.Linear, false);
